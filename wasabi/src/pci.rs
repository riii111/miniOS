...
pub struct Pci {
    ecm_ragne: Range<usize>,
}
impl Pci {
    pub fn new(mcfg: &AcpiMcfgDescriptor) -> Self {
        // To simpify, assume that there is one mcfg entry that maps all the
        ...
        Self {
            ecm_ragne: pci_config_space_base..pci_config_space_end,
        }
    }
    pub fn ecm_base<T>(&self, id: BusDeviceFunction) -> *mut T {
        (self.ecm_ragne.start + ((id.id as usize) << 12)) as *mut T
    }
